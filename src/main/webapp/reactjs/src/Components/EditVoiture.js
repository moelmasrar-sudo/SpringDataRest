...
import { faSave, faUndo, faEdit, faPlus, faUserPlus } from "@fortawesome/free-solid-svg-icons";
...
class EditVoiture extends Component {
        ...
        axios.get('/proprietaires')
            .then(response => {
                ...
                items.forEach(p => {
                    ...
                    const id = selfHref ? String(selfHref.split("/").pop()) : undefined;
                    if (id && !seenIds.has(id)) {
                        seenIds.add(id);
                        uniqueProprietaires.push({ ...p, id });
                    }
                });
                const update = { proprietaires: uniqueProprietaires };
                if (newId) update.proprietaireId = String(newId);
                this.setState(update);
            })
            .catch(error => console.error("ERROR fetching owners: " + error));
    }

    fetchVoitureDetails = (id) => {
        axios.get(`/voitures/${id}`)
            .then(response => {
                const { marque, modele, couleur, immatricule, prix, annee, proprietaire } = response.data;
                
                let proprietaireId = '';
                if (proprietaire && (proprietaire.id || proprietaire.id === 0)) {
                    proprietaireId = String(proprietaire.id);
                } else if (response.data._links && response.data._links.proprietaire) {
                    const propHref = response.data._links.proprietaire.href;
                    proprietaireId = String(propHref.split("/").pop());
                }
                
                this.setState({ marque, modele, couleur, immatricule, prix, annee, proprietaireId });
            })
            .catch(err => console.error("Error fetching voiture details", err));
    }
    ...
    submitVoiture = (event) => {
        ...
        const selectedProprietaire = this.state.proprietaires.find(p => String(p.id) === String(this.state.proprietaireId));
        ...
        const voiture = {
            ...
            proprietaire: selectedProprietaire // Full object for the custom controller
        };
                    ...
                    <Col md={10} lg={8}>
                        <Card className="border-0 shadow-sm rounded-3">
                            <Card.Header className="bg-white border-bottom-0 pt-4 px-4">
                                ...
                                <p className="text-muted small mt-2 mb-0">Mettez à jour les informations du véhicule sélectionné.</p>
                            </Card.Header>
                                ...
                                <Card.Body className="p-0">
                                    <Row className="mb-3">
                                        <Form.Group as={Col} md="6">
                                            ...
                                            <Form.Control required name="marque" value={this.state.marque} className="bg-light border-0" onChange={this.voitureChange} />
                                        </Form.Group>
                                        <Form.Group as={Col} md="6">
                                            <Form.Label className="fw-semibold small text-uppercase">Modèle</Form.Label>
                                            <Form.Control required name="modele" value={this.state.modele} className="bg-light border-0" onChange={this.voitureChange} />
                                        </Form.Group>
                                    </Row>
                                    <Row className="mb-3">
                                        <Form.Group as={Col} md="4">
                                            <Form.Label className="fw-semibold small text-uppercase">Couleur</Form.Label>
                                            <Form.Control required name="couleur" value={this.state.couleur} className="bg-light border-0" onChange={this.voitureChange} />
                                        </Form.Group>
                                        <Form.Group as={Col} md="4">
                                            <Form.Label className="fw-semibold small text-uppercase">Immatricule</Form.Label>
                                            <Form.Control required name="immatricule" value={this.state.immatricule} className="bg-light border-0" onChange={this.voitureChange} />
                                        </Form.Group>
                                        <Form.Group as={Col} md="4">
                                            <Form.Label className="fw-semibold small text-uppercase">Année</Form.Label>
                                            <Form.Control required name="annee" type="number" value={this.state.annee} className="bg-light border-0" onChange={this.voitureChange} />
                                        </Form.Group>
                                    </Row>
                                    <Row className="mb-4">
                                        <Form.Group as={Col} md="6">
                                            <Form.Label className="fw-semibold small text-uppercase">Prix (DH)</Form.Label>
                                            <Form.Control required name="prix" type="number" value={this.state.prix} className="bg-light border-0" onChange={this.voitureChange} />
                                        </Form.Group>
                                        <Form.Group as={Col} md="6">
